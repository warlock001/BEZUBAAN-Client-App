...
export default function EditPet({ route, navigation }) {
    const [petType, setPetType] = useState(route.params.pet.petType);
    const [breed, setBreed] = useState(route.params.pet.breed);
    const [age, setAge] = useState(route.params.pet.age);
    const [color, setColor] = useState(route.params.pet.color);
    const [petDescription, setPetDescription] = useState(route.params.pet.petDescription);
    const [phone, setPhone] = useState(route.params.phone);
    const [dialCode, setDialCode] = useState(route.params.dialCode);
    ...
    async function sendData() {
        ...
        if (!petType || !breed || !color || !petDescription || !age) {
            ...
            axios({
                method: 'PUT',
                url: `${REACT_APP_BASE_URL}/adoption?id=${route.params.pet._id}`,
                data: form,
                headers: {
                ...
                <ScrollView style={styles.bottomSection}>
                    <View style={{ height: '100%', padding: 24 }}>
                        <SafeAreaView style={{ marginBottom: 20 }}>
                            <SelectBox
                                defaultValue={route.params.pet.petType}
                                data={petTypes}
